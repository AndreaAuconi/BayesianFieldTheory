...
h[t_, s_] := -sigma * Sqrt[alpha] * (Abs[t] + Abs[s]);

g[t_, s_] := Piecewise[{
    {Exp[sigma^2 * Abs[s]], t > 0 && s < 0},
    {Exp[sigma^2 * Abs[t]], t < 0 && s > 0},
    {Exp[sigma^2 * Min[t, s]], t > 0 && s > 0},
    {Exp[sigma^2 * (Abs[t] + Abs[s] + Min[Abs[t], Abs[s]])], t < 0 && s < 0}
}];

f[t_, s_] := Sign[t] * Sign[s] * Exp[h[t, s]] * g[t, s];
